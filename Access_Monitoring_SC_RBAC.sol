...
contract AM_SC_RBAC is AccessControl//, Ownable
{
    ...
    mapping (address => User_RBAC_Session) User_Sessions;
    ...
    {
        //mapping (uint256 => Role_Struct) Required_Role_Identifier;
    ...
    function Add_Role(string memory Role, string memory Info, string[] memory Resources, int[] memory Resource_Permissions) public
    {
        ...
        //bytes32[] memory Resource_Hash = new bytes32[](Resources.length);
    ...
    function User_Request_Access(string memory Access_Request) public returns (bool Access)
    {
        ...
        //bytes32 Required_Role = Systems[System_Hash].Required_Role_Identifier;  //fetch the role required to access the system
    ...
    //evaluate the user access to the system, takes the user address
    function Access_Request_Deny(address User_Add, string memory Requested_System) private
    {
        uint256 Descision_Time = block.timestamp;   //get the current time from the block
        uint256 Last_Fail_Time = Users_Info[User_Add].Monitor_Behavior.Last_Request_Time;   //fetch last time the user had a failed request
        uint256 Time_Diff = Descision_Time - Last_Fail_Time;    //calculate the times between requests

        Time_Diff = Time_Diff / 1 hours;    // compute the time in hours
    ...
    function Block_User_Abuse(address User_Add, string memory Requested_System) private
    {
        ...
        string memory Abuse = string(abi.encodePacked(Strings.toString(block.timestamp),",",Strings.toHexString(uint256(uint160(User_Add)), 20),",",Role_String,",",System_Role_String,",",Requested_System,",",Users_Info[User_Add].Monitor_Behavior.Last_Requested_Resource,",",Strings.toString(Users_Info[User_Add].Monitor_Behavior.User_Suspended_Count),";")); //generate log for SOC team
    ...
    //Create an RBAC session when a user is authenticated against a system, takes the hash of the system being accessed, the address of the user, and the hash of the role being used within the sesison
    ...
    function Resource_Access(string memory Resource, uint256 Requested_Permissions) public returns (bool Access)
    {
        ...
        for (uint256 i = 0; i<User_Sessions[User_Add].Session_Role_Hash.length; i++) //for the number of roles attributed to the session
        {
            ...
            if (RBAC_Roles[Session_Hash_Role].Allowed_Resources[Requested_Resource_Hash].Resource_Timestamp != 0)   // if the requested resource is allowed by the RBAC role
            {
                if ((Requested_Permissions == 1) && (RBAC_Roles[Session_Hash_Role].Allowed_Resources[Requested_Resource_Hash].Resource_Permissions.Read))   // if the permisions of the rbac role allow the user to perform read against this resource AND the user requests to read the resource
                {
                    return true;    //  enable the user to read the resource
                }
                else if ((Requested_Permissions == 2) && (RBAC_Roles[Session_Hash_Role].Allowed_Resources[Requested_Resource_Hash].Resource_Permissions.Write)) // if the permisions of the rbac role allow the user to perform write against this resource AND the user requests to write to the resource
                {
                    return true;    // enable the user to write to the resource
                }
                else if ((Requested_Permissions == 3) && (RBAC_Roles[Session_Hash_Role].Allowed_Resources[Requested_Resource_Hash].Resource_Permissions.Execute)) // if the permisions of the rbac role allow the user to perform executes against this resource AND the user requests to execute the resource
                {
    ...
    //set up the Security Operations Centre account, take the address of the SOC account
